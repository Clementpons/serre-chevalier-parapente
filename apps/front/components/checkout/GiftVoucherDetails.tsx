...
import { Gift, User, Mail, MessageSquare, Edit2, Save, X, Tag } from 'lucide-react';
...
interface GiftVoucherDetailsProps {
  participantData: GiftVoucherData;
  itemId: string;
  onUpdate: () => void;
}
...
export function GiftVoucherDetails({ 
        ...
        `${process.env.NEXT_PUBLIC_BACKOFFICE_URL}/api/cart/items/${itemId}`,
        {
          ...
          body: JSON.stringify({
            participantData: {
              ...
              voucherStageCategory: participantData.voucherStageCategory,
              voucherBaptemeCategory: participantData.voucherBaptemeCategory,
            }
          }),
        }
      );
  ...
  const getVoucherLabel = () => {
    if (participantData.voucherProductType === 'STAGE') {
      return `Stage ${participantData.voucherStageCategory}`;
    } else if (participantData.voucherProductType === 'BAPTEME') {
      return `Baptême ${participantData.voucherBaptemeCategory}`;
    }
      ...
      <CardContent className="p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className="text-cyan-600 hover:text-cyan-700 hover:bg-cyan-100 h-8"
          >
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs text-cyan-700">Produit offert</p>
          ...
          <div className="flex items-start gap-2">
            <User className="w-4 h-4 text-cyan-600 mt-0.5 flex-shrink-0" />
            <div>
              <p className="text-xs text-cyan-700">Bénéficiaire</p>
              <p className="font-medium text-cyan-900">
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs text-cyan-700">Notification email</p>
              <p className="font-medium text-cyan-900">
                {(participantData.notifyRecipient || participantData.personalMessage) ? (
                  <>
                    ...
                    {participantData.recipientEmail && (
                      <span className="block text-xs text-cyan-700 mt-1">
                        {participantData.recipientEmail}
                      </span>
                    )}
                  </>
